...
import { listenForSheetChanges, scanAllSheets } from "./helpers/insertStyleRule.native.js";
...
function createTamagui(configIn) {
  ...
  if (configIn.themes) {
    ...
    noThemes && (foundThemes = scanAllSheets(noThemes, tokensParsed)), IS_REACT_19 && process.env.TAMAGUI_SKIP_THEME_OPTIMIZATION || noThemes && listenForSheetChanges();
  }
...
${JSON.stringify(variable, null, 2)}`);
  ...
  if (setConfig(config), configureMedia(config), createdConfigs.set(config, !0), process.env.NODE_ENV === "test" && (globalThis.__tamaguiConfig = config), configListeners.size && (configListeners.forEach(function (cb) {
    return cb(config);
