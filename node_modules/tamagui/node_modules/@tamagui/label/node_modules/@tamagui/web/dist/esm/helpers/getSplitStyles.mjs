import { isIos, isClient, isWeb, useIsomorphicLayoutEffect, IS_REACT_19 } from "@tamagui/constants";
...
import { getDynamicVal, getOppositeScheme, extractValueFromDynamic } from "./getDynamicVal.mjs";
...
import { groupCollapsed, groupEnd } from "./consoleLog.mjs";
...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, elementType, startedUnhydrated, debug) => {
  ...
  for (const keyOg in props) {
    ...
    if (process.env.NODE_ENV === "development" && debug === "verbose" && groupEnd(), disableExpandShorthands || keyInit in shorthands && (keyInit = shorthands[keyInit]), keyInit === "className" || keyInit in usedKeys || asChild && webViewFlexCompatStyles[keyInit] === valInit) continue;
    ...
    if (process.env.NODE_ENV === "development" && debug === "verbose" && (groupEnd(), groupEnd(), groupCollapsed(`  \u{1F511} ${keyOg}${keyInit !== keyOg ? ` (shorthand for ${keyInit})` : ""} ${shouldPassThrough ? "(pass)" : ""}`), log({
      isVariant,
      valInit,
      shouldPassProp
    }), isClient && log({
    ...
    if (propMapper(keyInit, valInit, styleState, disablePropMap, (key, val) => {
      ...
      if (isPseudo = key in validPseudoKeys, isMedia = !isPseudo && isMediaKey(key), isMediaOrPseudo = !!(isMedia || isPseudo), isVariant = variants && key in variants, (inlineProps?.has(key) || process.env.IS_STATIC === "is_static" && inlineWhenUnflattened?.has(key)) && (viewProps[key] = props[key] ?? val), styleProps.noExpand && isPseudo || isHOC && (isMediaOrPseudo || parentStaticConfig?.variants?.[keyInit])) {
        passDownProp(viewProps, key, val, isMediaOrPseudo), process.env.NODE_ENV === "development" && debug === "verbose" && (groupCollapsed(` - passing down prop ${key}`), log({
          val,
          after: {
            ...viewProps[key]
          }
      ...
      if (isPseudo) {
        ...
        if (!shouldDoClasses || isExit || isEnter) {
          ...
          isExit && (isDisabled = !styleProps.isExiting), isEnter && componentState.unmounted === !1 && (isDisabled = !0), process.env.NODE_ENV === "development" && debug === "verbose" && (groupCollapsed("pseudo", key, {
            isDisabled
          }), log({
            pseudoStyleObject,
            isDisabled,
  ...
  if (!styleProps.noMergeStyle && !asChildExceptStyleLike) {
    const style = styleState.style;
    {
      ...
      if (styleProps.isAnimated && !animationDriver.supportsCSSVars && isReactNative) style && (viewProps.style = style);else if (isReactNative) {
        const cnStyles = {
          $$css: !0
        };
...
function addStyleToInsertRules(rulesToInsert, styleObject, startedUnhydrated = !1) {
  {
    ...
    !startedUnhydrated || !IS_REACT_19 ? shouldInsertStyleRules(identifier) && (updateRules(identifier, styleObject[StyleObjectRules]), rulesToInsert[identifier] = styleObject) : rulesToInsert[identifier] = styleObject;
  }
}
